...
export default function AccountDeleteScreen() {
  ...
  const { clearVehicles, vehicles } = useVehicles();
  ...
  const handleConfirmDelete = useCallback(() => {
    ...
    resetPreferences();
    setStep('done');
  }, [clearFavorites, clearVehicles, resetPreferences]);
      ...
      {step === 'done' ? (
        <View accessibilityRole="alert">
          ...
          <Text className="mt-2 text-[13px] font-semibold leading-5 text-slate-500">
            Garage vehicles and favorite parking spots were removed from this
            device, and preferences were reset to their defaults.
          </Text>
          {account.isSignedIn ? (
            <Text className="mt-2 text-[13px] font-semibold leading-5 text-slate-500">
        ...
        <>
          ...
          <BulletLine
            text={`Garage vehicles (${vehicleCount.toLocaleString()} saved)`}
          />
          <BulletLine
          ...
          {step === 'confirm' ? (
            <>
              ...
              >
                This permanently removes the garage, favorite, and preference
                data stored on this device. This cannot be undone.
              </Text>
              <Pressable
                accessibilityHint="Permanently deletes garage, favorites, and preferences from this device"
